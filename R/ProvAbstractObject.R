...
#' @export
#'
#' @name ProvAbstractObject
#' @title PROV abstract class
#' @description This class models an PROV abstract object
#' @keywords PROV
#' @return Object of \code{ \link[R6]{R6Class}} for modelling an PROV abstract Object
#' @format \code{ \link[R6]{R6Class}} object.
#'
#' @note abstract class used internally by \pkg{prov4R}
#'
#' @author Emmanuel Blondel <emmanuel.blondel1@@gmail.com>
#'
ProvAbstractObject <- R6Class("ProvAbstractObject",
  inherit = prov4RLogger,
  private = list(
    ...
    xmlComments = function(compliant = NA){
      comments <- list()
      prov4R <- packageDescription("prov4R")
      title <- paste0("PROV-XML generated by prov4R R package - Version ", prov4R$Version)

      isCompliant <- ifelse(is.na(compliant),"NOT TESTED", ifelse(compliant, "YES", "NO"))
      compliance <- paste0("PROV-XML compliance: ", isCompliant)

      createdOn <- paste0("Creation date/time: ", format(Sys.time(), "%Y-%m-%dT%H:%M:%S"))
      author <- gsub(">","",gsub("<","",unlist(strsplit(as.character(eval(parse(text=prov4R$Authors)))," \\["))[1]))
      author <- paste0("\tContact: ", author)
      infoPage <- paste0("\tURL: ", prov4R$URL)
      bugReport <- paste0("\tBugReports: ", prov4R$BugReports)
      idx <- 1
      comments[[idx]] <- createdOn; idx <- idx+1
      comments[[idx]] <- title; idx <- idx+1
      comments[[idx]] <- compliance; idx <- idx+1
      comments[[idx]] <- paste("prov4R R package information:", author, infoPage, bugReport, sep="\n")
      return(comments)
    },
  ...
  public = list(
    ...
    #'@description Initializes an object of class \link{ProvAbstractObject}
    ...
    initialize = function(xml = NULL, element = NULL, namespace = NULL,
      ...
      self$namespace = getProvNamespace(private$xmlNamespacePrefix)
      self$attrs = attrs
    ...
    delListElement = function(field, metadataElement){
      startNb <- length(self[[field]])
      if(self$contains(field, metadataElement)){
        self[[field]] = self[[field]][!sapply(self[[field]], ProvAbstractObject$compare, metadataElement)]
      }
      endNb = length(self[[field]])
      return(endNb == startNb-1)
    },
    ...
    contains = function(field, metadataElement){
      ...
      if(length(self[[field]]) == 0){
        ...
        out = any(sapply(self[[field]], function(x){
          ProvAbstractObject$compare(x, metadataElement)
        }))
      }
      return(out)
    },
    ...
    decode = function(xml){
      ...
      for(child in xml_children){
        ...
        if(!is(child, "XMLInternalTextNode")){
          fieldClass <- ProvAbstractObject$getClassByNode(child)
            ...
            if(length(children)>0){
              if(length(children)==1){
                ...
                if(!is(childroot, "XMLInternalTextNode")){
                  child <- childroot
                  fieldClass <- ProvAbstractObject$getClassByNode(childroot)
                }
              }
            }
    ...
    encode = function(addNS = TRUE, validate = TRUE, strict = FALSE, encoding = "UTF-8"){
      ...
      if(self$isDocument()){
        rootNamespaces <- sapply(getProvNamespaces(), function(x){x$getDefinition()})
        ...
        if(addNS){
          ...
          if(!("xsi" %in% names(nsdefs))) nsdefs <- c(nsdefs, ProvNamespace$XSI$getDefinition())
          if(!("xlink" %in% names(nsdefs))) nsdefs <- c(nsdefs, ProvNamespace$XLINK$getDefinition())
      ...
      for(field in fields){
        ...
        if(field != "value"){
          ...
          if(!is.null(klass)){
            ns <-  ProvNamespace[[klass$private_fields$xmlNamespacePrefix]]$getDefinition()
          }
        }
      ...
      #validation vs. PROV-XML schemas
      compliant <- NA
      if(validate){
    ...
    validate = function(xml = NULL, strict = FALSE){
      ...
      if(!isValid){
        ...
        msg <- sprintf("Object '%s' is INVALID according to PROV-XML schemas!", self$getClassName())
        if(strict){
          self$ERROR(msg)
          stop(msg)
        }else{
        ...
        self$INFO(sprintf("Object '%s' is VALID according to PROV-XML schemas!", self$getClassName()))
      }
    ...
    getNamespaceDefinition = function(recursive = FALSE){
            ...
            if(hasContent){
              ...
              if(is(xObj, "ProvAbstractObject")){
                ...
                invisible(lapply(xObj, function(xObj.item){
                  nsdef.item <- NULL
                  if(is(xObj.item, "ProvAbstractObject")){
      ...
      invisible(lapply(names(self$attrs), function(attr){
        ...
        if(length(str)>1){
          nsprefix <- str[1]
          namespace <- ProvNamespace[[toupper(nsprefix)]]
...
ProvAbstractObject$compare = function(metadataElement1, metadataElement2){
  text1 <- NULL
  if(is(metadataElement1, "ProvAbstractObject")){
  ...
  text2 <- NULL
  if(is(metadataElement2, "ProvAbstractObject")){
...
ProvAbstractObject$getClasses = function(extended = FALSE, pretty = FALSE){
  getClassesInheriting(classname = "ProvAbstractObject", extended = extended, pretty = pretty)
}

ProvAbstractObject$getClassByNode = function(node){
  outClass <- NULL
...
getClassesInheriting <- function(classname, extended = FALSE, pretty = FALSE){
  list_of_classes <- ls(getNamespaceInfo("prov4R", "exports"))
  if(extended) {
    search_envs <- search()
    search_envs <- search_envs[search_envs!="package:prov4R"]
  ...
  list_of_classes <- list_of_classes[sapply(list_of_classes, function(x){
    ...
    if(is(clazz, "try-error")) clazz <- try(eval(parse(text=paste0("prov4R::",x))),silent=TRUE)
    r6Predicate <- class(clazz)[1]=="R6ClassGenerator"
    if(!r6Predicate) return(FALSE)

    provObjPredicate <- FALSE
    superclazz <- clazz
    while(!provObjPredicate && !is.null(superclazz)){
      clazz_fields <- names(superclazz)
      if(!is.null(clazz_fields)) if(length(clazz_fields)>0){
        if("get_inherit" %in% clazz_fields){
          superclazz <- superclazz$get_inherit()
          prov4RPredicate <- FALSE
          if("parent_env" %in% clazz_fields) prov4RPredicate <- environmentName(superclazz$parent_env)=="prov4R"
          provObjPredicate <- superclazz$classname == classname && prov4RPredicate
        }else{
          break
        }
      }
    }
    return(provObjPredicate)
  })]
  ...
  if(pretty){
    std_info <- do.call("rbind",lapply(list_of_classes, function(x){
      ...
      std_info <- data.frame(
        ...
        ns_uri = ProvNamespace[[clazz$private_fields$xmlNamespacePrefix]]$uri,
...
#' @name getProvClasses
#' @aliases getProvClasses
#' @title getProvClasses
#' @export
#' @description get the list of PROV classes, ie classes extending \link{ProvAbstractObject} super class,
#' including classes eventually defined outside \pkg{prov4R}. In case the latter is on the search path,
#' the list of PROV classes will be cached for optimized used by \pkg{prov4R} encoder/decoder.
#'
#' @usage getProvClasses()
#'
#' @examples
#'   getProvClasses()
#'
...
#
getProvClasses <- function(){
  if("package:prov4R" %in% search()){
    if(is.null(.prov4R$classes)){
      .prov4R$classes <- getClassesInheriting(classname = "ProvAbstractObject", extended = TRUE, pretty = FALSE)
    }
    return(.prov4R$classes)
  }else{
    getClassesInheriting(classname = "ProvAbstractObject", extended = TRUE, pretty = FALSE)
  }
}
